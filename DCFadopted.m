...
binshifts = Index - M ;                                                     % Calculate the pos of peak, remember with corr index -3,-2,-1,0,1,2,3...
...
for i = 0:frames-1
    ...
    [maz_energy,energy_index]= max(overall_energy);
      ...
      j = 4;
    while(1)
        if energy_index+k > size(ISAR,2)
            k = k - 1;
            j = j + 1;
        elseif energy_index - j < 1
            k = k + 1;
            j = j - 1;
        else 
            break;
    ...
    fn = sum(20*log10(abs(ISAR(:,energy_index-j:energy_index+k))),1);
    N = length(fn);
    pn = fn/(sum(fn));
    n = -N/2:1:N/2-1;
    fdc_formation = pn.*n;
    fdc = (PRF/(length(fn)))*sum(fdc_formation);
    %% Zero Doppler Removal
    size(ISAR,1);
    zero_doppler = size(ISAR,1)/2;
    B = abs(ISAR).^2;
    % Bandwidth to ignore
    B_zdm = round(30*(size(B,1)/PRF))    ;
    % looks away from the 0 Doppler going in the positive direction
    positive_zdm = B(zero_doppler+B_zdm:end,:);
    % looks away from the 0 Doppler going in the negative direction
    B_flipped = flipud(B);
    negative_zdm = B_flipped(zero_doppler+B_zdm:end,:);
    % Average away from the zero Doppler
    C = mean(mean(vertcat(positive_zdm,negative_zdm)));
    D = B - C;
    positive_zdm_D = D(zero_doppler+B_zdm:end,:);
    D_flipped = flipud(D);
    negative_zdm_D = D_flipped(zero_doppler+B_zdm:end,:);
    D_zdm = vertcat(positive_zdm_D,negative_zdm_D).^2;
    E = sqrt(mean(mean(D_zdm)));
    Contrast = E/C;
    contrast_matrix(1,i+1) = abs(fdc);
